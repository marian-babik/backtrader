...
from backtrader.stores import gdaxstore


class MetaGDAXData(DataBase.__class__):
    def __init__(cls, name, bases, dct):
        """Class has already been created ... register"""
        # Initialize the class
        super(MetaGDAXData, cls).__init__(name, bases, dct)

        # Register with the store
        gdaxstore.GDAXStore.DataCls = cls


class GDAXData(with_metaclass(MetaGDAXData, DataBase)):
    """GDAX Data Feed.
    ...
    Params:
      ...
      - ``historical`` (default: ``False``)
        ...
        larger than the one allowed by GDAX given the timeframe/compression
        chosen for the data.
      ...
      - ``backfill_from`` (default: ``None``)
        ...
        backfilling from GDAX will take place. This is ideally meant to backfill
    ...
    def setenvironment(self, env):
        ...
        belongs to"""
        super(GDAXData, self).setenvironment(env)
        env.addstore(self.o)

    def start(self):
        """Starts the GDAX connection"""
        super(GDAXData, self).start()
    ...
    def _st_start(self, instart=True, timeout=None):
        if self.p.historical:
            ...
            self.qhist = self.o.candles(
                ...
                self._timeframe, self._compression, timeout=self.p.reconntimeout)
    ...
    def _load(self):
        ...
        while True:
            if self._state == self._ST_LIVE:
                ...
                dtend = dateutil.parser.parse(msg['time'], ignoretz=True)

                self.qhist = self.o.candles(self.p.dataname, dtbegin, dtend,
                                            self._timeframe, self._compression,
                                            timeout=self.p.reconntimeout)
                self._state = self._ST_HISTORBACK
    ...
    def _load_tick(self, msg):
        dtobj = dateutil.parser.parse(msg['time'])
        dt = date2num(dtobj)
    ...
    def _load_history(self, msg):
        ...
        # Put the prices into the bar
        self.lines.open[0] = float(msg[3])
        self.lines.high[0] = float(msg[2])
        self.lines.low[0] = float(msg[1])
