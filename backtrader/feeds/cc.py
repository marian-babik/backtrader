...
from backtrader.stores import ccstore


class MetaCCData(DataBase.__class__):
    def __init__(cls, name, bases, dct):
        """Class has already been created ... register"""
        # Initialize the class
        super(MetaCCData, cls).__init__(name, bases, dct)

        # Register with the store
        ccstore.CCStore.DataCls = cls


class CCData(with_metaclass(MetaCCData, DataBase)):
    ...
    def setenvironment(self, env):
        ...
        belongs to"""
        super(CCData, self).setenvironment(env)
        env.addstore(self.o)

    def start(self):
        """Starts the Poloniex connection """
        super(CCData, self).start()
    ...
    def _st_start(self, instart=True, timeout=None):
        if self.p.historical:
            ...
            self.qhist = self.o.candles(self.p.dataname, dtbegin, dtend,
                                        self._timeframe, self._compression, exchange=self.p.exchange)

            self._state = self._ST_HISTORBACK
            return True

        self.qlive = self.o.streaming_prices(self.p.dataname, tmout=self.p.reconntimeout,
                                             compression=self._compression, exchange=self.p.exchange)
        if instart:
    ...
    def _load_history(self, msg):
        ...
        if self.p.quote_volume:
            self.lines.volume[0] = float(msg['volumefrom'])
        else:
            self.lines.volume[0] = float(msg['volumeto'])
