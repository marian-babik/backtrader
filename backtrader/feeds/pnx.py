...
from backtrader.stores import pnxstore


class MetaPNXData(DataBase.__class__):
    def __init__(cls, name, bases, dct):
        """Class has already been created ... register"""
        # Initialize the class
        super(MetaPNXData, cls).__init__(name, bases, dct)

        # Register with the store
        pnxstore.PNXStore.DataCls = cls


class PNXData(with_metaclass(MetaPNXData, DataBase)):
    """Poloniex Data Feed.
    ...
    def setenvironment(self, env):
        ...
        belongs to"""
        super(PNXData, self).setenvironment(env)
        env.addstore(self.o)

    def start(self):
        """Starts the Poloniex connection """
        super(PNXData, self).start()
        ...
        while True:
            if self._state == self._ST_LIVE:
                ...
                if self.p.live_wcandles:
                    dtend = datetime.utcfromtimestamp(msg['date'])
                else:
    ...
    def _load_tick(self, msg):
        ...
        if self.p.ask:
            tick = float(msg['lowestAsk'])
        elif self.p.price:
            tick = float(msg['last'])
        else:
            tick = float(msg['highestBid'])
        self.lines.open[0] = tick
        self.lines.high[0] = tick
        self.lines.low[0] = tick
        self.lines.close[0] = tick
        if self.p.price:
            self.lines.volume[0] = float(msg['baseVolume'])
        else:
            self.lines.volume[0] = 0.0
        self.lines.openinterest[0] = 0.0
    ...
    def _load_history(self, msg):
        dtobj = datetime.utcfromtimestamp(msg['date'])
        dt = date2num(dtobj)
        if dt <= self.lines.datetime[-1]:
            return False  # time already seen
        ...
        # Common fields
        self.lines.datetime[0] = dt
        if self.p.quote_volume:
            self.lines.volume[0] = float(msg['quoteVolume'])
        else:
